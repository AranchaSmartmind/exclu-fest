...
-- v184 · firma de compatibilidad para clientes que invoquen el RPC sin p_test_day.
drop function if exists public.start_coto_quiz(text);
create or replace function public.start_coto_quiz(p_festival_slug text)
returns jsonb
language sql
security definer
set search_path=public
as $$
  select public.start_coto_quiz(p_festival_slug, null::integer);
$$;
grant execute on function public.start_coto_quiz(text) to authenticated;

-- Recarga explícita de la caché de PostgREST.
select pg_notify('pgrst','reload schema');

-- COMPROBACIÓN: al terminar deben aparecer DOS filas para start_coto_quiz y UNA para finish_coto_quiz.
select p.proname as funcion,
       pg_get_function_identity_arguments(p.oid) as parametros
from pg_proc p
join pg_namespace n on n.oid=p.pronamespace
where n.nspname='public'
  and p.proname in ('start_coto_quiz','finish_coto_quiz')
